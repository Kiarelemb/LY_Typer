        ...
        for (int i = 0, l = this.subscriptInstances.length; i < l; i++) {
            ...
            if (this.subscriptInstances[i].isUseSign()) {
                ...
                for (int x = i; x <= this.subscriptInstances[i].getNext(); ) {
                    ...
                    tcsd.add(new TipCharStyleData(x, word, code, TextStyleManager.STYLE_TYPE[0],
                            TextStyleManager.BOLD_FALSE));
                    x += word.length();
...
//                if (code.endsWith("_") && codeLen == maxLen && i != this.subscriptInstances.length - 1) {
        ...
        for (char c : codeChars) {
            if (c == 'b') {
                if (TypingData.bModel == 1) {
                    rightCounts++;
                } else {
                    leftCounts++;
                }
            } else if (TypingData.LEFT.indexOf(c) != -1) {
                leftCounts++;
            } else if (TypingData.RIGHT.indexOf(c) != -1) {
                rightCounts++;
            } else if (c == '_') {
                spaceCounts++;
                if (TypingData.spaceModel == 1) {
                    rightCounts++;
                } else {
                    leftCounts++;
                }
        ...
        TipData.StandardData data = new TipData.StandardData(first, multi, singleCounts, phraseTypeCounts, oneFirst,
                oneMulti, twoFirst, twoMulti, threeFirst, threeMulti, fourFirst, fourMulti, totalCounts, leftCounts,
                rightCounts, spaceCounts);
